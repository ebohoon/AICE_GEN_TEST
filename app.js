...
**［ 작성 과제 ］**
위 지문을 바탕으로 생성형 AI에 입력할 프롬프트를 작성하시오. 답안은 반드시 다음 5개 항목으로 구분하여 작성하시오.
**# 역할   # 맥락   # 명령   # 형식   # 어조**
...
**［ 작성 과제 ］**
위 지문을 바탕으로 AI 에이전트의 시스템 프롬프트를 작성하시오. 답안은 반드시 다음 5개 항목으로 구분하여 작성하시오.
**# 역할   # 맥락   # 명령   # 형식   # 어조**
...
최종 결과물은 조건에 맞는 핵심 메시지 3개와 각 메시지의 핵심 포인트 설명으로 구성되어야 한다. 핵심 메시지는 1~2문장, 50자 내외로 간결하게 작성되어야 하며, 경영진 보고용으로 적합한 격식 있고 신뢰감 있는 어조를 유지해야 한다.${TASK_PROMPT}`,
  ...
  "A-Q2": {
    ...
    body:
`온라인 직무교육 플랫폼 **런업클래스(RunUp Class)**는 직장인을 대상으로 데이터 분석, 생성형 AI, 업무 자동화 등 실무 중심 온라인 강의를 제공하고 있다.

최근 런업클래스는 신규 과정인 **"생성형 AI 업무 활용 입문 패키지"**를 출시했으며, 사전 신청 고객에게는 7일간 무료로 강의를 체험할 수 있는 혜택을 제공했다.
...
실제로는 무료 체험 신청 후 발송되는 안내 메일에서 수강 시작 버튼을 눌러야 강의실이 활성화되는 구조였지만, 안내 문구가 충분히 명확하지 않아 고객이 혼란을 느낀 상황이다.

런업클래스 **CS 운영 담당자 이서연 주임**은 고객에게 보낼 답변 문구를 작성해야 한다. 답변에는 고객이 불편을 겪은 점에 대한 사과, 강의실 활성화 방법 안내, 추가 도움이 필요한 경우 고객센터가 지원하겠다는 내용이 포함되어야 한다. 단, 고객의 불만을 가볍게 여기거나 "안내 메일을 확인하지 않은 고객 책임"처럼 보이는 표현은 피해야 한다.

최종 결과물은 문자 또는 카카오톡으로 보낼 수 있는 고객 응대 문구 2가지 버전으로 작성되어야 한다. 하나는 기본 안내형, 다른 하나는 조금 더 공감이 강조된 문구로 작성되어야 한다. 각 문구는 너무 길지 않게 5~7문장 이내로 작성하며, 정중하고 친절한 어조를 유지해야 한다.${TASK_PROMPT}`,
  },
  "B-Q1": {
    ...
    body:
`통신·플랫폼 기업 **온링크커넥트(OnLink Connect)**는 기업 고객을 대상으로 클라우드, 네트워크, 데이터 플랫폼, AI 자동화 솔루션을 제공하고 있다.

온링크커넥트는 다음 분기 전략 발표회에서 자사의 새로운 브랜드 방향을 **"AI 기반 B2B 디지털 전환 파트너"**로 제시할 예정이다.
...
이미지에는 스마트 팩토리, 금융, 물류, 헬스케어 등 다양한 산업을 상징하는 요소가 포함되어야 하며, 중앙에는 AI 코어 또는 데이터 허브처럼 보이는 시각 요소가 배치되어야 한다.
...
• **Content**: 이미지에 포함되어야 할 핵심 대상, 배경, 사물, 인물, 상징 요소
• **Action/Atmosphere**: 장면에서 느껴지는 분위기, 흐름, 움직임, 감정
• **Style**: 이미지의 시각적 스타일, 디자인 방향, 색감, 표현 방식
• **Technical Specs**: 화면 비율, 해상도, 구도, 여백, 카메라 앵글 등 기술적 조건

최종 결과물은 ① AI에게 이미지 생성 프롬프트를 만들어 달라고 요청하는 프롬프트, ② 그 결과로 완성하여 실제 이미지 생성에 사용한 이미지 생성 프롬프트(CAST 4요소 반영), ③ 해당 프롬프트로 생성한 이미지로 구성되어야 한다.`,
  },
  "C-Q1": {
    no: "4번", title: "디지털 헬스케어 경쟁사 제휴 현황 조사 프롬프트",
    body:
`디지털 헬스케어 스타트업 **메디핏랩스(MediFit Labs)**는 만성질환 관리 앱과 웨어러블 기기 연동 서비스를 제공하고 있다.

최근 국내외 디지털 헬스케어 시장에서는 병원, 보험사, 제약사, 웨어러블 기업과의 제휴를 통해 서비스 신뢰도를 높이고 고객 접점을 확대하는 사례가 늘어나고 있다.

메디핏랩스의 **파트너십 전략 담당자 강민수 대리**는 자사의 향후 제휴 전략을 수립하기 위해 주요 경쟁사의 파트너십 사례를 조사해야 한다. 조사 대상은 디지털 헬스케어 또는 원격진료, 웨어러블 건강관리, 만성질환 관리 영역에서 활동하는 국내외 기업 4개 이상이다.

조사 내용에는 각 기업의 주요 제휴 파트너, 제휴 목적, 서비스 또는 사업에 미친 영향, 메디핏랩스가 참고할 수 있는 시사점이 포함되어야 한다.

자료는 가능한 한 공식 보도자료, 기업 홈페이지, 공시자료, 신뢰도 높은 산업 리포트 등을 우선 활용해야 하며, 출처가 불분명한 블로그나 개인 의견 중심 자료는 피해야 한다.

최종 결과물은 경쟁사별 비교표와 핵심 인사이트 요약으로 구성되어야 한다. 표에는 기업명, 주요 제휴 파트너, 제휴 목적, 전략적 의미, 참고 출처가 포함되어야 한다. 어조는 내부 전략 검토 보고서에 적합하도록 객관적이고 분석적으로 작성되어야 한다.${TASK_PROMPT}`,
  },
  "C-Q2": {
    no: "5번", title: "글로벌 AI 교육 플랫폼 시장 조사 프롬프트",
    body:
`AI 기반 교육 서비스 기업 **에듀브릿지AI(EduBridge AI)**는 성인 직장인을 대상으로 생성형 AI 활용 교육, 직무별 AI 튜터, 실습형 온라인 코스를 제공하는 신규 서비스를 기획하고 있다.

최근 글로벌 시장에서는 Coursera, Udemy, Khan Academy, Duolingo, LinkedIn Learning 등 여러 교육 플랫폼이 AI 기능을 도입하며 학습 추천, 개인화 피드백, AI 튜터, 자동 평가 기능을 강화하고 있다.

에듀브릿지AI의 **AI서비스 기획 담당자 최유진 매니저**는 글로벌 AI 교육 플랫폼의 서비스 전략을 조사하여 신규 서비스 기획에 참고하려고 한다. 조사 대상은 AI 기반 학습 기능을 제공하거나, 기존 교육 서비스에 생성형 AI 기능을 접목한 해외 교육 플랫폼 4개 이상이다.

조사 내용에는 각 플랫폼의 주요 AI 기능, 대상 학습자, 수익 모델, 차별화 요소, 에듀브릿지AI가 참고할 수 있는 서비스 기획 시사점이 포함되어야 한다.

자료는 기업 공식 홈페이지, 공식 블로그, 보도자료, 신뢰도 높은 교육·기술 매체를 중심으로 수집해야 한다.
...
최종 결과물은 플랫폼별 비교표와 에듀브릿지AI의 신규 서비스 기획에 참고할 수 있는 시사점으로 구성되어야 한다. 단순 소개가 아니라, "어떤 기능을 참고할 수 있는지", "어떤 차별화 전략이 필요한지"가 드러나야 한다. 어조는 서비스 기획 회의에서 활용할 수 있도록 실무적이고 분석적으로 작성되어야 한다.${TASK_PROMPT}`,
  ...
  "D-Q1": {
    ...
    body:
`교육 서비스 기업 **케이러닝그룹(K-Learning Group)**은 여러 교육 사업부를 운영하고 있으며, 사업부별 매출에 영향을 주는 주요 요인을 파악하려고 한다.

**데이터 분석 담당자 윤태호 사원**은 각 사업부의 연간매출액과 여러 운영 지표 간의 관계를 분석해야 한다.
...
윤태호 사원은 annual_sales와 나머지 변수들 간의 피어슨 상관계수를 계산하여, 연간매출액과 가장 관련성이 높은 변수를 찾고자 한다.

분석 결과는 보고서에 붙여 넣기 쉽도록 간단한 표로 정리되어야 하며, 마지막에는 가장 높은 상관관계를 보인 변수명과 상관계수를 한 줄로 제시해야 한다.
...
상관계수는 소수점 둘째 자리까지 반올림해야 하며, 최종 답안은 변수명,상관계수 형식으로 작성되어야 한다. 예를 들어 마케팅비의 상관계수가 0.86으로 가장 높다면 mkt_spend,0.86과 같이 작성한다.${TASK_PROMPT}`,
  ...
  "D-Q2": {
    ...
    body:
`온라인 교육 기업 **케이에듀링크(KEdulink)**는 월별 수강생 수 데이터를 관리하고 있다.

**데이터 운영 담당자 한서진 주임**은 분석 보고서를 작성하기 전, 수강생수 데이터에 포함된 결측값과 이상값을 처리해야 한다.
...
최종 결과물에는 결측값이 있었던 행 번호, 이상값으로 판단된 행 번호, 대체에 사용한 중앙값, 전처리 후 전체 평균 수강생수가 포함되어야 한다.${TASK_PROMPT}`,
  ...
  "E-Q1": {
    ...
    body:
`산업 자동화 솔루션 기업 **로보인사이트(RoboInsight)**는 산업용 로봇, 자동화 설비, AI 기반 공정 분석 솔루션을 제공하고 있다.

로보인사이트 전략기획팀은 매주 산업 뉴스, 경쟁사 보도자료, 투자 동향, 정부 정책 발표, 고객 산업별 기술 트렌드를 확인하여 주간 시장 분석 보고서 초안을 작성하고 있다.

**시장분석 담당자 오현우 대리**는 매주 여러 사이트를 직접 확인하며 자료를 수집하고 있지만, 수작업에 시간이 많이 걸리고 자료 선별 기준도 일정하지 않다.

또한 수집한 자료 중 어떤 내용을 보고서에 우선 반영해야 하는지 판단하는 데 시간이 오래 걸린다.
...
최종 시스템 프롬프트에는 에이전트 이름을 먼저 기재하고, 자료 수집 대상, 경쟁사 자료 선별 기준, 데이터 정상 여부 확인 기준, 핵심 이슈 요약 기준, 보고서 초안 구성 항목이 포함되어야 한다.

보고서 초안 구성 항목은 자료 수집 결과, 선별된 경쟁사 자료, 데이터 정상 여부 확인 결과, 핵심 시장 이슈 요약, 보고서 반영 필요 사항을 포함해야 한다. 어조는 전략기획팀 내부 보고서에 적합하도록 객관적이고 분석적이어야 한다.${TASK_AGENT}`,
  },
  "E-Q2": {
    ...
    body:
`건강기능식품 브랜드 **바이탈그린(VitalGreen)**은 신제품 **슬립밸런스** 출시를 앞두고 기존 고객 정보를 활용하여 Power Automate 기반의 이메일 마케팅 자동화 플로우를 구축하였다.
