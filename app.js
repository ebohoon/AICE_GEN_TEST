...
분석에는 첨부된 엑셀 데이터 파일인 preprocess_data.xlsx 파일을 사용한다. 데이터 첨부는 해당 데이터 파일에 있는 값을 복사하여 프롬프트에 추가하여 사용한다. 최종 출력은 필요한 값만 깔끔하게 제시되어야 하며, 불필요한 설명이나 중간 계산 과정은 포함하지 않는다.`,
  ...
  "E-Q1": {
    no: "8번", title: "주간 시장 분석 보고서 자동화 AI 에이전트 시스템 프롬프트 작성",
    ...
    body:
`산업용 로봇 솔루션 기업 **로보인사이트(RoboInsight)**는 제조업 고객을 대상으로 산업용 로봇, 자동화 설비, AI 기반 공정 분석 솔루션을 제공하고 있다. 로보인사이트 전략기획팀은 매주 외부 시장 동향과 경쟁사 움직임을 분석하여 주간 시장 분석 요약 보고서 초안을 작성하고 있다.

전략기획팀의 **시장분석 담당자 오현우 대리**는 매주 월요일 오전마다 산업 뉴스, 경쟁사 보도자료, 투자 동향, 정부 정책 발표, 고객 산업별 기술 트렌드를 직접 확인한다. 이후 수집한 자료 중 로보인사이트의 사업과 관련성이 높은 내용을 선별하고, 핵심 시장 이슈와 경쟁사 전략을 요약하여 팀장에게 보고한다.

현재 이 업무는 반복성이 높고 시간이 많이 소요된다. 자료 수집, 경쟁사 관련 자료 선별, 데이터 정상 여부 확인, 핵심 이슈 요약, 전략적 시사점 정리, 보고서 초안 작성까지 대부분 수작업으로 이루어지고 있다. 또한 담당자마다 자료 선별 기준이 달라 보고서 품질의 일관성이 떨어지는 문제가 있다. 단순 뉴스 요약과 실제 전략적 의미가 있는 이슈가 섞여 보고서의 우선순위가 불명확해지는 경우도 있다.

이에 따라 로보인사이트 전략기획팀은 주간 시장 분석 업무를 자동화하는 **AI 리서치 에이전트**를 제작하려고 한다. 이 에이전트는 단순히 뉴스를 요약하는 역할이 아니라, 산업용 로봇, 제조 자동화, AI 공정 분석, 스마트팩토리 분야의 자료를 수집하고, 로보인사이트 사업과의 관련성을 판단하며, 경쟁사 동향과 시장 이슈를 구조화된 보고서 초안으로 정리해야 한다.

AI 에이전트가 수행해야 할 업무는 다음과 같다.
• 외부 시장 동향 데이터 수집
• 경쟁사 관련 자료 선별
• 수집 데이터 정상 여부 확인
• 핵심 시장 이슈 및 경쟁사 전략 요약
• 요약 내용 적정성 확인
• 주간 시장 분석 요약 보고서 초안 작성

자료를 선별할 때는 로보인사이트의 사업 영역인 산업용 로봇, 제조 자동화, AI 공정 분석, 스마트팩토리와 직접 관련된 내용을 우선해야 한다. 단순 홍보성 기사나 사업 관련성이 낮은 자료는 제외해야 하며, 공식 보도자료, 정부 발표자료, 투자 공시, 신뢰도 높은 산업 전문 매체 자료를 우선 활용해야 한다.

수집 데이터의 정상 여부도 확인해야 한다. 자료의 출처, 발행일, 작성 주체, 원문 URL을 확인하고, 동일한 내용이 중복 수집되지 않았는지 검토해야 한다. 기업명, 제품명, 투자금액, 정책명, 날짜 등 핵심 정보에 오류가 없는지도 확인해야 한다. 출처가 불명확하거나 최신성이 낮은 자료는 제외하거나 "확인 필요"로 표시해야 한다.

보고서 초안에는 이번 주 핵심 시장 이슈, 주요 경쟁사 동향, 기술·정책·투자 관련 변화, 로보인사이트에 미칠 수 있는 영향, 후속 검토가 필요한 사항이 포함되어야 한다. 단, AI 에이전트는 최종 보고서를 확정하는 것이 아니라, 전략기획팀 담당자가 검토할 수 있는 내부 검토용 초안을 작성하는 역할을 수행해야 한다.

작성할 프롬프트는 "프롬프트를 만들어달라"는 요청문이 아니라, AI 에이전트의 설정창이나 시스템 프롬프트에 그대로 입력할 수 있는 형태여야 한다. 따라서 "당신은 로보인사이트 전략기획팀의 주간 시장 분석 AI 리서치 에이전트입니다"처럼 에이전트의 역할을 직접 부여하는 방식으로 작성해야 한다.

**［ 작성 과제 ］**
위 지문을 바탕으로 주간 시장 분석 보고서 자동화 AI 에이전트의 시스템 프롬프트를 직접 작성하시오. 프롬프트에는 반드시 다음 5가지 요소가 포함되어야 한다.
• 역할  • 맥락  • 명령  • 형식  • 어조

또한 다음 내용이 프롬프트에 드러나도록 작성하시오.
• AI 에이전트가 수행해야 할 단계별 업무 절차
• 자료 선별 기준
• 수집 데이터 정상 여부 확인 기준
• 요약 내용의 적정성 검토 기준
• 최종 보고서 초안의 구성 항목
• 확인된 사실과 분석적 해석을 구분해야 한다는 조건
• 과장된 표현이나 출처가 불명확한 내용은 제외하거나 별도 표시해야 한다는 조건

최종 답안은 AI 에이전트 제작에 바로 활용할 수 있는 시스템 프롬프트 형태로 작성하시오.`,
  },
  "E-Q2": {
...
function applyQuestionMode() {
  const noAI = !!(QUESTIONS[current] && QUESTIONS[current].noAI);
  const imgMode = !noAI && isImageQuestion(current);
  const llmBox = $("#llmBox");
  const imgBox = $("#imgBox");
  const hint = $("#modeHint");
  ...
  if (noAI) {
    ...
    if (hint) {
      ...
      hint.innerHTML = "📝 <strong>이 문항은 AI 모델을 사용하지 않습니다.</strong> 우측 AI 플레이그라운드는 비활성화되며, 좌측 <strong>‘프롬프트 입력’</strong>란에 직접 작성한 시스템 프롬프트를 입력한 뒤 제출하세요.";
    }
    return;
  }
