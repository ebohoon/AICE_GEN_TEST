...
/* DB row → API 응답용 세션 객체(카멜/명세 필드명으로 정규화) */
...
async function getSession(id) {
  const { rows } = await sql`select * from exam_session where session_id = ${id}`;
  return toSession(rows[0]);
}

/* 일회용 진입 토큰 소모 처리 — 아직 미사용이고 만료 전이면 성공.
 * 반환: 소모된 세션(정상 진입) 또는 null(이미 사용/만료/없음) */
